...
public class AsmSimulator extends ActionBarActivity{
    ...
    public Object getCaarbacSystemService(String serviceName) {
        ...
        if(!appPermission.equals(ALLOW)){
            throw new SecurityException("The application: \""+callingAppPackageName+"\" is trying to access \""+serviceName+"service\" without being granted permission");
        }
        else {
    ...
    private Object getService(String serviceName) {
        ...
        switch (serviceName) {
            case Context.LOCATION_SERVICE:
                service = callerContext.getSystemService(serviceName);
                break;
            case "MEDIA_RECORDER":
